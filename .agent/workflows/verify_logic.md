...
2.  **테스트 이미지 존재**: `test/images/` 디렉토리에 인물별 폴더와 이미지들이 있어야 합니다.
3.  **Puppeteer 설치**: `npm install` 또는 `yarn`으로 의존성이 설치되어 있어야 합니다.

## 2. 실행 명령 (Execution)

다음 쉘 명령어를 실행하여 검증 스크립트를 구동하십시오. (인물별 폴더를 자동으로 탐색하여 검사합니다.)

```bash
node test/verify-consistency.js
```
...
- **✅ PASS**: 해당 피험자(Subject)의 모든 사진(x장)이 동일한 직업 결과를 반환했습니다.
