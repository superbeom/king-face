...
이 워크플로우는 `king-face` 프로젝트의 모든 컨텍스트 파일을 로드하고, 개발 에이전트로서 준수해야 할 규칙을 재확립합니다.

## 1. Load Context Files
우선 다음 파일들을 순차적으로 읽고 내용을 메모리에 확실히 적재하십시오.
(파일이 존재하지 않으면 건너뛰지 말고 생성 여부를 물어보십시오.)

- `king-face/project_context.md` (프로젝트 개요 및 아키텍처, 규칙)
- `king-face/project_history.md` (히스토리 및 Q&A)
- `king-face/task.md` (작업 목록)
- `king-face/future_todos.md` (기술 부채 및 아이디어)
- `king-face/git_convention.md` (커밋 컨벤션)
- `king-face/git_branch_strategy.md` (브랜치 전략)
- `king-face/walkthrough.md` (구현 상세 내역)

## 2. Summarize & Internalize
읽어들인 내용을 바탕으로 다음을 수행하여 사용자에게 보고하십시오:
1. **프로젝트 요약**: 현재 프로젝트의 목표, 핵심 아키텍처(Client-Side AI 등), 기술 스택을 간략히 요약.
...
2.  **기술 부채 관리 (Debt Management)**:
    ...
    - 예: `// TODO: 성능 개선 필요` -> `future_todos.md`에 `[ ] 성능 개선 필요 (파일명)` 추가.
