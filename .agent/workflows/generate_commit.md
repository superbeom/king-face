...
1.  `king-face/git_convention.md` 파일을 읽고 다음 규칙을 숙지하십시오:
...
2.  `king-face/commit_msg.sh` 파일을 생성하고(덮어쓰기), 아래와 같은 형식의 Bash 스크립트를 작성하십시오.
...
3.  생성된 파일에 실행 권한을 부여하십시오: `chmod +x king-face/commit_msg.sh`

## 4. Final Report
- 사용자에게 "✅ 커밋 메시지 준비 완료! 아래 명령어로 커밋을 확정하세요:" 라는 메시지와 함께 `./king-face/commit_msg.sh` 명령어를 안내하십시오.
