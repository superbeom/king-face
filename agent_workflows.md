...
채팅창에 아래 명령어를 입력하세요:
```bash
@king-face/.agent/workflows/restore_context.md
```
...
2. 채팅창에 아래 명령어를 입력하세요:
   ```bash
   @king-face/.agent/workflows/generate_commit.md
   ```

### 📋 실행되는 작업
1. `git diff --cached`를 분석하여 변경 내용을 파악합니다.
2. 컨벤션에 맞춰 영문/한글 커밋 메시지를 작성합니다.
3. `king-face/commit_msg.sh` 파일을 생성합니다.
4. 사용자는 터미널에서 `./king-face/commit_msg.sh`만 입력하면 커밋이 완료됩니다.
